    ...
    <div className="px-4 pt-4 pb-4">
      ...
      <div className="flex gap-2 mb-5">
        ...
        >
          <div className="flex items-center gap-2">
            <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
              exportType === 'missing' ? 'border-orange-400' : 'border-gray-200'
            }`}>
              {exportType === 'missing' && <div className="w-2 h-2 rounded-full bg-orange-400" />}
            </div>
            <span className={`text-sm font-semibold ${exportType === 'missing' ? 'text-orange-700' : 'text-gray-500'}`}>
              Faltantes
            </span>
          </div>
          <p className={`text-2xl font-bold mt-2 ${exportType === 'missing' ? 'text-orange-500' : 'text-gray-300'}`}>
        ...
        >
          <div className="flex items-center gap-2">
            <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
              exportType === 'duplicates' ? 'border-violet-500' : 'border-gray-200'
            }`}>
              {exportType === 'duplicates' && <div className="w-2 h-2 rounded-full bg-violet-500" />}
            </div>
            <span className={`text-sm font-semibold ${exportType === 'duplicates' ? 'text-violet-700' : 'text-gray-500'}`}>
              Repetidas
            </span>
          </div>
          <p className={`text-2xl font-bold mt-2 ${exportType === 'duplicates' ? 'text-violet-500' : 'text-gray-300'}`}>
            {duplicateStickers.length}
